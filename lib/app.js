...
const app = net.createServer(socket => {
  socket.on('data', data => {
    ...
    if(request.method === 'GET' && request.path === '/') {
      ...
      socket.end(createResponse({ body: '<html><body><h1>red</h1></body></html>' }));
    } else if(request.method === 'GET' && request.path === '/green') {
      socket.end(createResponse({ body: '<html><body><h1>green</h1></body></html>' }));
    } else if(request.method === 'GET' && request.path === '/blue') {
      socket.end(createResponse({ body: '<html><body><h1>blue</h1></body></html>' }));
    } else 
